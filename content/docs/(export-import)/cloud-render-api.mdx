...
- **richTextEnabled**: boolean — enable [Rich Text](/docs/rich-text) rendering for text elements (default: false).
- **htmlTextRenderEnabled**: boolean — **DEPRECATED:** Use `richTextEnabled` instead. Still supported for backward compatibility.
...
For PNG and JPEG exports, use `dpi` to embed resolution metadata and `dpiMetadata` to control embedding behavior (`'auto'`, `'always'`, or `'never'`).

**Note:** The legacy parameter `htmlTextRenderEnabled` is still supported but deprecated. Use `richTextEnabled` instead.
