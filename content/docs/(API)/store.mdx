...
Store is a **root data model** object that you can use to read and control canvas content, check selection, export into different formats and a lot more! It will also provide you with access to [Pages](https://polotno.com/docs/page-overview) and [Elements](https://polotno.com/docs/element-overview).
...
Set role of the current user. Possible values are: `user` and `admin`. For more information see [Roles Documentation](https://developer.polotno.com/docs/user-roles).
...
The function to add a new custom font to the user project. Added fonts will be saved into exported JSON. If you want to add fonts available globally (ignored in JSON export), take a look into [Fonts Tutorial](https://developer.polotno.com/docs/config#how-to-change-available-fonts).
