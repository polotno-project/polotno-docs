...
**Note:** Calling `store.openSidePanel('')` before the `<SidePanel>` component mounts will not work, because the component resets the panel to its default on mount. To hide the side panel on initial load, use the `defaultSection=""` prop on the `<SidePanel>` component instead. See [Side Panel overview](/docs/side-panel-overview) for details.
...
### store.loadJSON(json, keepHistory?) / store.loadJSON(json, options?)
