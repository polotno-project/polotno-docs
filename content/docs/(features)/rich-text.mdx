...
<Callout type="note">
`unstable_useHtmlTextRender` is deprecated. Always enable rich text with `setRichTextEnabled`.
</Callout>

If you use Cloud Render API, please make sure to pass `richTextEnabled: true` to enable rich mode.

<Callout type="warning">
**Deprecated:** The parameter `htmlTextRenderEnabled` is deprecated in favor of `richTextEnabled`. Both parameters are currently supported for backward compatibility, but `htmlTextRenderEnabled` will be removed in a future version.
</Callout>
...
Yes. When using [Cloud Render API](/docs/cloud-render-api) or [polotno-node](/docs/server-side-image-generation-with-node-js), pass `richTextEnabled: true` in export options. (The legacy parameter `htmlTextRenderEnabled` is also supported but deprecated.)
